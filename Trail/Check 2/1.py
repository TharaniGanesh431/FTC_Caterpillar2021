...
#path = r'F:\Projects\Caterpillar\Trail\Check 1\4\Input11.jpg'
#img=cv2.imread(path)
...
print("Pic Co-ordinate Execution: ",end_pic-start_pic)

(y, x) = np.where(mask==255)
(topy, topx) = (np.min(y), np.min(x))
(bottomy, bottomx) = (np.max(y), np.max(x))

print("\nNo. of contours",length)
...
if length ==2:
    ...
    if topx < 100 and (bottomx1-bottomx)>100:
        print("\n\nBlock: 1")
    elif topx > 100 and (bottomx1-bottomx)<100:
        print("\n\nBlock: -1")
    else:
        print("****************Condition Not Satisfied**************")
    print("\npic x: ",topx1,bottomx1)
    print("\nblock x: ",topx,bottomx)
